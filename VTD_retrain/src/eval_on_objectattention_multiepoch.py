...
from dataset import ObjectAttention
from config import *
...
logs_folder = '/projects/ObjectAttention_retrain/logs'
latest_subdir = max(glob.glob(os.path.join(logs_folder, '*/')), key=os.path.getmtime)
print("Latest subdir: "+ latest_subdir)
epoch_weights = sorted(os.listdir(latest_subdir))
...
def test():
	...
	# Prepare data
	print("Loading Data")
	val_dataset = ObjectAttention(ObjectAttention_val_data, ObjectAttention_val_label,
	...
	for weight in epoch_weights:
		...
		snapshot = torch.load(os.path.join(latest_subdir,weight))
		...
		print("\tAUC:{:.4f}"
			  ...
			  format(torch.mean(torch.tensor(AUC)),
					 torch.mean(torch.tensor(distance)),
					 evaluation.ap(in_vs_out_groundtruth, in_vs_out_pred)))
		epoch_mean_auc = torch.mean(torch.tensor(AUC))
		epoch_auc.append(epoch_mean_auc.item())
		epoch_mean_dist = torch.mean(torch.tensor(distance))
		epoch_dist.append(epoch_mean_dist.item())
	print(epoch_auc)
	print(epoch_dist)
	plt.subplot(2,1,1)
	plt.plot(list(range(1,len(epoch_weights)+1)), epoch_auc)
	plt.grid(axis='both')
	plt.xticks(np.arange(min(list(range(1,len(epoch_weights)+1))), max(list(range(1,len(epoch_weights)+1)))+1,1))
	plt.xlabel('epochs')
	plt.ylabel('AUC')
	plt.subplot(2,1,2)
	plt.plot(list(range(1,len(epoch_weights)+1)), epoch_dist)
	plt.grid(axis='both')
	plt.xlabel('epochs')
	plt.ylabel('dist')
	plt.xticks(np.arange(min(list(range(1,len(epoch_weights)+1))), max(list(range(1,len(epoch_weights)+1)))+1,1))
	plt.savefig("auc_dist_bs4_cs2.png")
