...
class ObjectAttention(Dataset):
    def __init__(self, data_dir, annotation_dir, transform, input_size=input_resolution, output_size=output_resolution,
                 test=False, imshow=False, seq_len_limit=400):
        sessions = glob.glob(os.path.join(annotation_dir, '*'))
        #print(sessions)
        self.all_sequence_paths = []
        for s in sessions:
            sequence_annotations = glob.glob(os.path.join(s, '*', '*.txt'))
        ...
        #print(self.all_sequence_paths)

    def __getitem__(self, index):
        sequence_path = self.all_sequence_paths[index]
        #print("Sequence path:" + str(sequence_path))
        df = pd.read_csv(sequence_path, header=0, dtype={'frame':str},
                         names=['frame', 'xmin', 'ymin', 'xmax', 'ymax', 'gazex', 					'gazey'])                
        session_name = sequence_path.split('/')[-3]
        #print("Session name:" + str(session_name))
        setting = sequence_path.split('/')[-2]
        #print("Setting name:" + str(setting))
        seq_len = len(df.index)
        ...
        if not self.test:
            ...
            if cond_crop < 0.5:
                ...
                # Get image size
                frame_name = str(df['frame'].iloc[0]) + '.ppm'
                path = os.path.join(self.data_dir, session_name, setting, frame_name)
                #print("Path for images: " + str(path))
                img = Image.open(path)
                img = img.convert('RGB')
                width, height = img.size
        ...
        for i, row in df.iterrows():
            ...
            name_ppm = row['frame'] + '.ppm'
            impath = os.path.join(self.data_dir, session_name, setting, name_ppm)
            #print("impath: "+ impath)
            img = Image.open(impath)
            img = img.convert('RGB')
